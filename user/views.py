...
def checkout(request):
    ...
    for x in request.session["cart"].values():
        ...
        if x["issuing_buying"]=="Issuing":
            doi=datetime.now()+timedelta(days=14)
    ...
    with connection.cursor() as cursor:
        ...
        if not flag:
            ...
            if click=="0":
                ...
                data["issuing_buying"]="Issuing"
                request.session["issuing_count"]+=1
                data["issued_at"]=datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
...
def commit_transaction(request,choice_id):
    ...
    with connection.cursor() as cursor:
        if request.session["buying_count"]==0 and request.session["issuing_count"]>0:
            ...
            data["rdate"]=doi.strftime("%m/%d/%Y, %H:%M:%S")
            data=str(data)    
            sql="INSERT INTO transactions(uid,type,started_at) values({},{},'{}')".format(request.session["uid"],0,datetime.now())
            #return HttpResponse(sql)
            cursor.execute(sql)
            # cursor.commit()
            cursor.execute("SELECT tid from transactions where uid = {} order by tid DESC LIMIT 1 ".format(request.session["uid"]))
            ...
            if choice_id==0:
                ...
                data["mop"]="Cash"
            sql="INSERT INTO transactions(uid,type,started_at) values({},{},'{}')".format(request.session["uid"],1,datetime.now())
            #return HttpResponse(sql)
            cursor.execute(sql)
            # cursor.commit()
            cursor.execute("SELECT tid from transactions where uid = {} order by tid DESC LIMIT 1 ".format(request.session["uid"]))
